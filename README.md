...
### 4. Запуск в ручном режиме (для разработки)
...
⏳ Pending (1):
  • @newuser — 123456 — 2025-02-01

✅ Approved (3):
  • @user1 — 111111 — 2025-01-15
  ...

/approve newuser (админ)
✅ Пользователь @newuser одобрен.
```
...
Telegram → bot.py → OpenAI API
                ↓
            S3 Storage
        - {chat_id}.json → история чата
        - {chat_id}_settings.json → модель пользователя
        - {ADMIN_CHAT_ID}_users.json → пользователи
```

- **bot.py** — основной файл бота (~450 строк)
...
| Проблема | Решение |
|----------|---------|
| Бот не отвечает | Проверьте статус: `/users` — ожидаете ли вы подтверждения? |
